...
export default function RuleConfigurator() {
	...
	const handleChangeConditionType = (ruleId: string, newType: ConditionType) => {
		setRules((prev) =>
			prev.map((r) => (r.id === ruleId ? { ...r, condition: { conditionType: newType, config: {} } } : r))
		);
	};
						...
						title={
							<Stack direction="row" alignItems="center" justifyContent="space-between">
								...
								<Box>
									...
									<IconButton onClick={() => handleMoveRule(index, 1)} disabled={index === rules.length - 1}>
										<ArrowDownward />
									</IconButton>
					...
					<CardContent>
						...
						<Stack direction="row" spacing={2} mt={1} mb={3}>
							<Select
								...
								onChange={(e) => handleChangeConditionType(rule.id, e.target.value as ConditionType)}
								sx={{ width: 250 }}
							>
							...
							{rule.condition.conditionType === "userJoinedVoice" && (
								<TextField
									...
									onChange={(e) => handleConditionConfigChange(rule.id, "categoryName", e.target.value)}
								/>
							)}
							{rule.condition.conditionType === "dashboardSetupClick" && (
								<TextField
									...
									onChange={(e) => handleConditionConfigChange(rule.id, "setupKey", e.target.value)}
								/>
							)}
						</Stack>
						...
						{rule.actions.map((action, aIndex) => (
							<Stack key={aIndex} direction="row" spacing={2} sx={{ mt: 1, mb: 1 }}>
								<Select
									...
									onChange={(e) => handleChangeActionType(rule.id, aIndex, e.target.value as ActionType)}
									sx={{ width: 220 }}
								>
								...
								{action.actionType === "sendPrivateMessage" && (
									<TextField
										...
										onChange={(e) => handleActionConfigChange(rule.id, aIndex, "message", e.target.value)}
									/>
								)}
								{action.actionType === "askTracking" && (
									<TextField
										...
										onChange={(e) => handleActionConfigChange(rule.id, aIndex, "question", e.target.value)}
									/>
								)}
								{action.actionType === "channelChoice" && (
									<TextField
										...
										onChange={(e) => handleActionConfigChange(rule.id, aIndex, "label", e.target.value)}
									/>
								)}
