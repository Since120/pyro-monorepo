				...
				<Box sx={{ flex: 1, minWidth: "300px" }}>
					<Stack spacing={2}>
						...
						<Box sx={{ bgcolor: "background.paper", minHeight: 10 }}>
							<TiptapEditor content={embed.description} onContentChange={handleDescriptionUpdate} />
						</Box>
						...
						{embed.fields.map((f, idx) => (
							<Box key={idx} sx={{ border: "1px solid #666", p: 1, borderRadius: 1, mb: 1 }}>
								<Stack spacing={1}>
									<TextField
										label="Field Name"
										value={f.name}
				...
				{(embed.footerText || embed.timestamp) && (
					...
					>
						...
						<Box>
							...
							{embed.timestamp && (
								<span style={{ marginLeft: embed.footerText ? 8 : 0 }}>• {new Date().toLocaleString()}</span>
							)}
						</Box>
					</Box>
				)}
